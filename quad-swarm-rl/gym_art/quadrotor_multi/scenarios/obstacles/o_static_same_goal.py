...
class Scenario_o_static_same_goal(Scenario_o_base):
    ...
    def reset(self, obst_map=None, cell_centers=None, spawns=None, goals=None):
        ...
        if spawns is None or goals is None:
            self.start_point = self.generate_pos_obst_map_2(num_agents=self.num_agents)
            self.end_point = self.max_square_area_center()
        else:
            self.start_point = copy.deepcopy(spawns)
            self.end_point = copy.deepcopy(goals)
