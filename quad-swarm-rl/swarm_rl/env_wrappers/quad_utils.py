...
def make_quadrotor_env_multi(cfg, render_mode=None, **kwargs):
    ...
    env = QuadrotorEnvMulti(
        ...
        # Rendering
        quads_view_mode=cfg.quads_view_mode[:2], quads_render=cfg.quads_render,
        # Quadrotor Specific (Do Not Change)
        dynamics_params=quad, raw_control=raw_control, raw_control_zero_middle=raw_control_zero_middle,
        dynamics_randomize_every=dyn_randomize_every, dynamics_change=dynamics_change, dyn_sampler_1=sampler_1,
        sense_noise=sense_noise, init_random_state=False,
        init_mode=cfg.quads_init_mode,
        init_dataset_path=resolve_repo_path(cfg.quads_init_dataset_path),
        custom_init_indices=cfg.quads_init_custom_indices,
        seed=cfg.quads_seed,
        print_dataset_id=cfg.quads_print_dataset_id,
    )
    # ---- enable per-scene recording on the base env (only if rendering is on) ----
    if cfg.quads_render:
        env.enable_recording(out_dir=resolve_repo_path(getattr(cfg, "video_out_dir", "outputs/videos")),
                             fps=getattr(cfg, "video_fps", 30),
                             prefix=getattr(cfg, "video_prefix", "scene"))
